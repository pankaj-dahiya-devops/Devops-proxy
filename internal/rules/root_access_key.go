...
// RootAccessKeyExistsRule flags AWS accounts where the root account has active
...
type RootAccessKeyExistsRule struct{}

func (r RootAccessKeyExistsRule) ID() string   { return "ROOT_ACCESS_KEY" }
func (r RootAccessKeyExistsRule) Name() string { return "Root Account Has Active Access Keys" }

// Evaluate returns one CRITICAL finding when the root account has access keys.
func (r RootAccessKeyExistsRule) Evaluate(ctx RuleContext) []models.Finding {
	...
	return []models.Finding{
		{
			...
			ResourceType:   models.ResourceRootAccount,
			Region:         "global",
