...
// SavingsPlanUnderutilizedRule flags regions where Savings Plan coverage is
...
type SavingsPlanUnderutilizedRule struct{}

func (r SavingsPlanUnderutilizedRule) ID() string   { return spUnderutilizedRuleID }
func (r SavingsPlanUnderutilizedRule) Name() string { return "Savings Plan Underutilized" }

// Evaluate returns one Finding per SavingsPlanCoverage entry that has
// CoveragePercent < 60 and OnDemandCostUSD > 100.
func (r SavingsPlanUnderutilizedRule) Evaluate(ctx RuleContext) []models.Finding {
	...
	for _, cov := range ctx.RegionData.SavingsPlanCoverage {
		...
		findings = append(findings, models.Finding{
			...
			ResourceType:            models.ResourceSavingsPlan,
			Region:                  cov.Region,
