...
func TestS3DefaultEncryptionMissingRule_ID(t *testing.T) {
	r := S3DefaultEncryptionMissingRule{}
	if r.ID() != "S3_DEFAULT_ENCRYPTION_MISSING" {
		t.Error("unexpected rule ID")
	}
}

func TestS3DefaultEncryptionMissingRule_NilRegionData(t *testing.T) {
	findings := S3DefaultEncryptionMissingRule{}.Evaluate(RuleContext{})
	if findings != nil {
		t.Errorf("want nil with nil RegionData, got %v", findings)
	}
}

// TestS3DefaultEncryptionMissingRule_EncryptionEnabled_NoFinding verifies
// that a bucket with SSE configured is not flagged.
func TestS3DefaultEncryptionMissingRule_EncryptionEnabled_NoFinding(t *testing.T) {
	ctx := RuleContext{
	...
	findings := S3DefaultEncryptionMissingRule{}.Evaluate(ctx)
	if len(findings) != 0 {
		t.Errorf("want 0 findings for encrypted bucket, got %d", len(findings))
	}
}

// TestS3DefaultEncryptionMissingRule_NoEncryption_HighSeverity verifies that
// a bucket without SSE is flagged with HIGH severity at region "global".
func TestS3DefaultEncryptionMissingRule_NoEncryption_HighSeverity(t *testing.T) {
	ctx := RuleContext{
		AccountID: "111122223333",
		Profile:   "test",
		RegionData: &models.RegionData{
	...
	if f.ResourceType != models.ResourceS3Bucket {
...
func TestS3DefaultEncryptionMissingRule_NoBuckets(t *testing.T) {
	ctx := RuleContext{
		AccountID:  "123",
...
func TestS3DefaultEncryptionMissingRule_MultipleBuckets(t *testing.T) {
	ctx := RuleContext{
		AccountID: "123",
		RegionData: &models.RegionData{
			Security: models.SecurityData{
