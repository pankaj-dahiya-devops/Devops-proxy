...
func TestSavingsPlanUnderutilizedRule_IDAndName(t *testing.T) {
	r := SavingsPlanUnderutilizedRule{}
...
func TestSavingsPlanUnderutilizedRule_NilRegionData(t *testing.T) {
	if got := (SavingsPlanUnderutilizedRule{}).Evaluate(RuleContext{}); got != nil {
		t.Errorf("expected nil for nil RegionData, got len=%d", len(got))
	}
}

func TestSavingsPlanUnderutilizedRule_Evaluate(t *testing.T) {
	const (
	...
	makeCtx := func(coverages ...models.SavingsPlanCoverage) RuleContext {
		return RuleContext{
			AccountID: account,
			Profile:   profile,
	...
	t.Run("coverage 30% and cost 500 is HIGH", func(t *testing.T) {
		ctx := makeCtx(models.SavingsPlanCoverage{
			Region:          region,
			CoveragePercent: 30.0,
			OnDemandCostUSD: 500.0,
		})
		findings := (SavingsPlanUnderutilizedRule{}).Evaluate(ctx)
		if len(findings) != 1 {
	...
	t.Run("coverage 70% is not flagged (above threshold)", func(t *testing.T) {
		...
		if got := (SavingsPlanUnderutilizedRule{}).Evaluate(ctx); len(got) != 0 {
	...
	t.Run("finding has correct fields", func(t *testing.T) {
		...
		if f.ResourceType != models.ResourceSavingsPlan {
			t.Errorf("ResourceType = %q; want %q", f.ResourceType, models.ResourceSavingsPlan)
		}
	...
	t.Run("only low-coverage regions with sufficient cost flagged from mixed set", func(t *testing.T) {
		ctx := makeCtx(
			models.SavingsPlanCoverage{Region: "us-east-1", CoveragePercent: 30.0, OnDemandCostUSD: 500.0},  // flagged: HIGH
			models.SavingsPlanCoverage{Region: "us-west-2", CoveragePercent: 75.0, OnDemandCostUSD: 500.0},  // not flagged: coverage OK
			models.SavingsPlanCoverage{Region: "eu-west-1", CoveragePercent: 50.0, OnDemandCostUSD: 50.0},   // not flagged: cost too low
			models.SavingsPlanCoverage{Region: "ap-south-1", CoveragePercent: 55.0, OnDemandCostUSD: 300.0}, // flagged: MEDIUM
		)
		findings := (SavingsPlanUnderutilizedRule{}).Evaluate(ctx)
		if len(findings) != 2 {
