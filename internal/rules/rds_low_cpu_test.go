...
func TestRDSLowCPURule_NilRegionData(t *testing.T) {
	if got := (RDSLowCPURule{}).Evaluate(RuleContext{}); got != nil {
		t.Errorf("expected nil for nil RegionData, got len=%d", len(got))
	}
}

func TestRDSLowCPURule_Evaluate(t *testing.T) {
	const (
	...
	makeCtx := func(instances ...models.RDSInstance) RuleContext {
		return RuleContext{
			AccountID: account,
			Profile:   profile,
	...
	t.Run("CPU 2% + cost 100 → HIGH, savings 30", func(t *testing.T) {
		ctx := makeCtx(models.RDSInstance{
			DBInstanceID:    "db-1",
			Region:          region,
	...
	t.Run("CPU 15% → not flagged", func(t *testing.T) {
		...
		if got := (RDSLowCPURule{}).Evaluate(ctx); len(got) != 0 {
	...
	t.Run("finding has correct fields", func(t *testing.T) {
		...
		findings := (RDSLowCPURule{}).Evaluate(makeCtx(inst))
		if len(findings) != 1 {
			t.Fatalf("want 1 finding, got %d", len(findings))
		}
		f := findings[0]
		...
		if f.ResourceType != models.ResourceRDS {
			t.Errorf("ResourceType = %q; want %q", f.ResourceType, models.ResourceRDS)
		}
	...
	t.Run("only available low-CPU instances with known cost flagged from mixed set", func(t *testing.T) {
		ctx := makeCtx(
			models.RDSInstance{DBInstanceID: "db-1", Region: region, Status: "available", AvgCPUPercent: 3.0, MonthlyCostUSD: 100.0},   // flagged: HIGH
			models.RDSInstance{DBInstanceID: "db-2", Region: region, Status: "available", AvgCPUPercent: 0.0, MonthlyCostUSD: 100.0},   // skipped: no CW data
			models.RDSInstance{DBInstanceID: "db-3", Region: region, Status: "available", AvgCPUPercent: 50.0, MonthlyCostUSD: 100.0},  // skipped: high CPU
			models.RDSInstance{DBInstanceID: "db-4", Region: region, Status: "stopped", AvgCPUPercent: 3.0, MonthlyCostUSD: 100.0},     // skipped: not available
			models.RDSInstance{DBInstanceID: "db-5", Region: region, Status: "available", AvgCPUPercent: 2.0, MonthlyCostUSD: 0},       // skipped: no cost data
			models.RDSInstance{DBInstanceID: "db-6", Region: region, Status: "available", AvgCPUPercent: 7.0, MonthlyCostUSD: 200.0},   // flagged: MEDIUM
		)
		findings := (RDSLowCPURule{}).Evaluate(ctx)
		if len(findings) != 2 {
